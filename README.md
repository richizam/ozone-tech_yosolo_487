...
- **Доказательства перцепции (панели для жюри):** панели «RGB + RTX-глубина +
  ...
  ролик `perception_demo.mp4` и видео прогонов — в Nextcloud.
...
- **Где лежат материалы.** По правилам сдачи код и лёгкие артефакты — в этом
  репозитории, тяжёлые файлы — в облачном хранилище Nextcloud.
  - **В репозитории (проверяется без скачивания):** весь код и конфигурации,
    консолидированные сводки прогонов и сырые журналы
    ([`matrix_summary.json`](docs/report/isaac_evidence/xbelt/matrix/matrix_summary.json),
    `summary.json` и `events.csv` по каждому прогону), матрица твина из 22 прогонов
    ([docs/report/validation/](docs/report/validation/)), машинные аудиты,
    панели перцепции и кадры сенсоров (PNG), итоговый отчёт, официальные STL.
    Этого достаточно, чтобы перепроверить каждое число из отчёта.
  - **В Nextcloud:** видеодемонстрация решения, набор видео прогонов
    (`videos_final/`, `twin_videos/`), экспортированные облака точек (`*.npy`),
    презентация. Ссылка на хранилище — в поле «Решение» на платформе.

## 9. Команда YOSOLO

| Участник | Роль | Вклад |
|---|---|---|
| **Рикардо  Самбрано (Ricardo Zambrano)** | Инженер-мехатроник | Механика ячейки и исполнительная часть tilt-tray; оба физических твина (Isaac Sim + MuJoCo) из единого источника `cell/params.py`; интеграция перцепции с исполнением |
