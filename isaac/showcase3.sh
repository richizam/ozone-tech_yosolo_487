#!/usr/bin/env bash
# Final video set v3 (user directive): PERFECT-condition shots from multiple
# camera angles + cinematic camera paths for the hackathon defense reel, plus
# the retained technical evidence. Clean nominal = seed 7 (no arm
# intervention, no stuck item). bash showcase3.sh [out_root]
set -u
OUT_ROOT="${1:-/root/sortmaster_out/showcase3}"
...
run() {
  ...
  docker run --rm --name "isaacrec-$name" --gpus all --network=host \
    ...
    --out "/workspace/sortmaster_out/showcase3/$name" \
...
# ---- PERFECT nominal (seed 7, no arm) from multiple STATIC angles
...
# ---- CINEMATIC camera paths (defense reel), seed 7 clean, matched duration
run cine_orbit  --seed 7 --camera overview --camera-path orbit  --path-secs 85 --depth-stills 0 --max-sim-s 400
run cine_dolly  --seed 7 --camera overview --camera-path dolly  --path-secs 85 --depth-stills 0 --max-sim-s 400
run cine_crane  --seed 7 --camera overview --camera-path crane  --path-secs 85 --depth-stills 0 --max-sim-s 400
# ---- retained TECHNICAL evidence (new materials/lighting), route close-ups
