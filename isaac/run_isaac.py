    ...
    while len(done) < n_total and t < args.max_sim_s:
        if step_i % decim == 0:
            ...
            for slug in list(active.keys()):
                ...
                wp_ = P.SORTER["wrap_pan"]
                in_wrap = (wp_["x0"] - 0.03 < p[0] < wp_["x1"] + 0.03
                           and wp_["y0"] - 0.03 < p[1] < wp_["y1"] + 0.03
                           and p[2] > wp_["z_top"] - 0.04)
                if (p[2] < 0.22 and dest is None and not in_wrap
                ...
                sp_ = P.SPILL
                on_spill = (not on_car
                            and sp_["z_top"] - 0.03 < p[2]
                            < sp_["z_top"] + 0.45
                            and any(x0 - 0.02 < p[0] < x1 + 0.02
                                    and y0 - 0.02 < p[1] < y1 + 0.02
                                    for x0, x1, y0, y1 in sp_["zones"]))
                if on_spill and st.get("spill_since") is None:
                    st["spill_since"] = t
                elif not on_spill:
                    st.pop("spill_since", None)
                if (st.get("spill_since") is not None
                        and t - st["spill_since"] > sp_["watch_s"]
                        and not st.get("spill_handled")):
                    st["spill_handled"] = True
                    ev(t, "spill_callout", slug,
                       pos=[round(float(v), 3) for v in p])
                    deliver(slug, "MANUAL", t)
                    continue
